...
function SelectPRepContainer(props: any) {
  ...
  if (
    queryViewer.data &&
    queryViewer.data.viewer &&
    queryViewer.data.viewer.username &&
    !delegations
  ) {
    jsonRpcCall("get_verify_info_by_id", {
      _id: queryViewer.data.viewer.username,
    }).then((result) => {
      const resultJson = JSON.parse(result);
      governanceCall("getDelegation", { address: resultJson.address }).then(
        (result2) => {
